...
export async function createProject(option) {
	...
	logger.info(`dir: ${dir}`)
	logger.info(`cwd: ${CWD}`)
	logger.info(resolve(CWD, 'lib/config/ui', uiVersion, 'package.json'))
				...
				if (err) {
					...
					new Promise((resolved, reject) => {
						...
						if (langVersion === 'javascript') {
							copy(resolve(CWD, 'lib/config/ui', uiVersion, 'js/package.json'), packagePath).then(() => {
								copy(
									resolve(CWD, 'lib/config/ui', uiVersion, 'js/vite.config.js'),
									viteConfigPath
								).then(() => {
									resolved()
								})
							...
							copy(resolve(CWD, 'lib/config/ui', uiVersion, 'ts/package.json'), packagePath).then(() => {
								copy(
									resolve(CWD, 'lib/config/ui', uiVersion, 'ts/vite.config.js'),
									viteConfigPath
								).then(() => {
									resolved()
								})
